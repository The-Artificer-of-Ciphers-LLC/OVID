...
class TestDiscSubmitChapters:
    ...
    def test_submit_with_chapters(self, client, auth_header, second_auth_header):
        """POST with chapters in a title -> 201; after a second contributor
        verifies it, the D-09 anti-echo redaction lifts and lookup returns
        ...
        # Second contributor re-submits identical structure -> auto-verify, so
        # titles (and their chapters) are no longer anti-echo redacted (D-09).
        verify_resp = client.post("/v1/disc", json=payload, headers=second_auth_header)
    ...
    def test_submit_without_chapters_backward_compat(self, client, auth_header, second_auth_header):
        """POST without chapters field -> 201; after verification titles carry
        ...
        # Second contributor verifies -> anti-echo redaction lifts (D-09).
        verify_resp = client.post("/v1/disc", json=payload, headers=second_auth_header)
        assert verify_resp.status_code == 200
