...
### G-07-3 — Apple Sign-In flow is non-functional (response_mode/scope/callback-method/SameSite)
status: open
severity: major
found_in: Test 5 (attempting to configure Apple as a 2nd provider); never live-tested (Phase-6 blocked live-provider UAT item)
reason: api/app/auth/routes.py apple_login sends `scope=name email` with `response_mode=query`, but Apple REQUIRES `response_mode=form_post` whenever name/email scope is requested — Apple rejects the authorize request with error -24007 ("response_mode must be form_post when name or email scope is requested"). Even if switched to form_post, three further problems: (a) the callback is `@auth_router.get("/apple/callback")` GET-only, but form_post invokes it via POST -> 405; (b) it reads `request.query_params.get("code")`, but form_post delivers code/state in the POST form body; (c) the state check pops `apple_state` from the SameSite=Lax session cookie, which browsers do NOT send on Apple's cross-site POST navigation -> state-mismatch 401. GitHub and Google are unaffected (standard code/query/GET flow).
chosen_fix: (deferred, needs design) switch apple_login to response_mode=form_post; change the callback to accept POST and read code/state from the form body (await request.form()); and carry/verify the CSRF state in a way that survives Apple's cross-site POST (e.g. a SameSite=None session cookie scoped to the API, or encode state without relying on the Lax session cookie). This is Phase-6 OAuth territory, not Phase-7 web UI; tracked here because it surfaced during Phase-7 UAT.
evidence: Apple docs/forums confirm form_post is mandatory with name/email scope (error -24007) and that the callback is then a POST.

### G-07-4 — Post-link logged-out UI in the option-B link-provider flow
status: open
severity: major
found_in: Test 5 (Settings -> Link a provider -> Google while already logged in)
reason: Server-side the link SUCCEEDS — resolve_auth (api/app/auth/merge.py branch 2, explicit link) attaches the provider and finalize_auth mints a valid token for the CORRECT account (create_access_token(user.id)); DB confirms google linked to the existing github_4738965 account (not a PendingAccountLink), and GET /v1/auth/me returns 200 for that account+token. Yet after the link the web UI showed logged-OUT and a manual refresh did NOT recover it; only a fresh sign-in fixed it. Two real fragilities identified from source: (1) web/lib/auth.ts useAuth calls clearToken() on ANY getMe() failure (the catch block), not just on 401/invalid-token — so a single transient/network failure permanently wipes the token and the logout survives refresh (the persistence symptom). (2) The option-B link flow (web/lib/api.ts linkProvider + api/app/auth/routes.py POST /v1/auth/link/{provider}) sets link_to_user_id via a cross-origin fetch's SameSite=Lax session cookie that must survive into a SEPARATE top-level navigation to /{provider}/login — a browser-dependent cookie hand-off (Safari ITP / Firefox Total Cookie Protection / Chrome partitioning) that can silently drop the link intent (degrading to a merge offer). NOTE: the auto-trace's "cookie lost -> logged in as wrong/new account" claim was disproven by the DB (clean branch-2 link to the correct account).
exact_trigger: not yet reproduced browser-side — the happy path should log the user in, so the logout came from a transient /me failure hitting fragility (1) or a partitioning edge on (2). Needs a devtools or headless-Playwright reproduction to pin precisely.
chosen_fix: (a) web/lib/auth.ts: only clearToken() when getMe returns 401/invalid-token; leave the token intact on network/transient errors (surface a retry/loading state instead). (b) Harden the link flow: carry the link intent as a server-signed, single-use, short-TTL token in the /login URL query, validated in finalize_auth, instead of relying on the cross-origin-fetch session cookie. (c) web/app/auth/callback/page.tsx: make the email_conflict branch use window.location.assign (match the success-path full-nav, same class as G-07-1).
